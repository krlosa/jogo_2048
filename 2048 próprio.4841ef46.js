...
"use strict";
const Game = require("824cc97830146efd");
const game = new Game();
...
render();
(0, _swipeJs.initializeSwipe)(handleMove);

},{"824cc97830146efd":"3wwaZ","./swipe.js":"esDKI"}],"3wwaZ":[function(require,module,exports,__globalThis) {
"use strict";
class Game {
    ...
    mergeLine(line) {
        ...
        for(let i = 0; i < filteredLine.length; i++)if (filteredLine[i] === filteredLine[i + 1]) {
            ...
            this.score += value;
            i++;
        } else mergedLine.push(filteredLine[i]);
    ...
    moveLeft() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            if (previousRow.some((cell, index)=>cell !== mergedRow[index])) hasChanged = true;
            this.state[row] = mergedRow;
        }
        if (hasChanged) this.spawnRandomTile();
        this.updateStatus();
    }
    moveRight() {
        if (this.status !== "playing") return;
    ...
    moveUp() {
        ...
        for(let col = 0; col < 4; col++){
            ...
            if (previousColumn.some((cell, index)=>cell !== mergedColumn[index])) hasChanged = true;
    ...
    start() {
        ...
        const [first, second] = emptyCells.sort(()=>Math.random() - 0.5).slice(0, 2);
...
},{}],"esDKI":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "initializeSwipe", ()=>initializeSwipe);
function initializeSwipe(callback) {
    let startX = 0;
    let startY = 0;
    ...
    document.addEventListener('touchend', (event)=>{
        ...
        if (Math.abs(diffX) > Math.abs(diffY)) {
            if (diffX > 0) callback('right');
            else callback('left');
        } else // Movimento vertical
        if (diffY > 0) callback('down');
        else callback('up');
    });
}
...
},{}]},["huHYX","3Aj1C"], "3Aj1C", "parcelRequire94c2", {})

//# sourceMappingURL=2048 próprio.4841ef46.js.map
